import { API_BASE_URL, ORS_API_KEY, ORS_API_URL } from '../constants';
import { Temple, TourPlan, HotelResult } from '../types';
...
export const getHotelAffiliateLink = async (hotelId: string): Promise<string | null> => {
  try {
    ...
    if (!response.ok) {
      throw new Error(`Failed to get affiliate link: ${response.status}`);
    }
