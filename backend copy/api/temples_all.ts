...
export default async function handler(req: VercelRequest, res: VercelResponse): Promise<void> {
  console.log('Temples All API handler called');
  ...
  try {
    ...
    console.log(`Querying temples collection with pagination - page: ${page}, limit: ${limit}, skip: ${skip}`);

    // Get total count for pagination info
    const totalCount = await db.collection<TempleDocument>("temples").countDocuments();
    const totalPages = Math.ceil(totalCount / limit);

    // Query with pagination
    const templeDocuments: TempleDocument[] = await db.collection<TempleDocument>("temples")
      .find({})
    ...
    console.log(`Found ${templeDocuments.length} temple documents (page ${page} of ${totalPages})`);
    ...
    console.log('Sending successful response with pagination info');
    ...
    console.error('==================== ERROR IN TEMPLE ALL API ====================');
