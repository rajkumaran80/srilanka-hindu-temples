...
export default async function handler(req: VercelRequest, res: VercelResponse): Promise<void> {
  console.log('Add approved Photo API handler called');
  ...
  try {
    ...
    console.log(`Adding approved photo for temple: ${templeId}, photo: ${photoName}`);

    // Add photo name to temple's photos array
    const updateResult = await (db.collection("temples") as any).updateOne(
      { _id: ObjectId.createFromHexString(templeId) },
      {
        $addToSet: { photos: photoName.trim() },
    ...
    if (updateResult.modifiedCount === 0) {
      console.log(`Photo already exists in photos for temple: ${templeId}`);
      return res.status(200).json({
        success: true,
        message: 'Photo already exists in approved photos',
        photoName: photoName.trim()
      });
    }

    console.log(`approved photo added successfully for temple: ${templeId}`);
    console.log('Sending successful response');
    res.status(201).json({
      success: true,
      message: 'Photo added to approved photos successfully',
      photoName: photoName.trim()
    });

  } catch (err: unknown) {
    console.error('==================== ERROR IN ADD APPROVED PHOTO API ====================');
