// api/presigned_upload_suggested_temple_photo.ts
// Vercel Serverless handler (TypeScript)
// Generates presigned URL (SAS) for uploading photo to Azure Blob Storage for suggested temples
...
export default async function handler(req: VercelRequest, res: VercelResponse): Promise<void> {
  try {
    ...
    const { templeId, fileType = "image/jpeg", filename } = requestBody;
    ...
    // connect to mongo and fetch suggested temple by id
    const client = await getMongoClient();
    const collection = client.db(MONGODB_DB).collection("suggested_temples");

    const queryId = parsePossibleObjectId(String(templeId));
    let suggestedTempleDoc: any = null;

    if (queryId instanceof ObjectId) {
      suggestedTempleDoc = await collection.findOne({ _id: queryId });
    } else {
      // Fallback search by other fields if needed
      suggestedTempleDoc = await collection.findOne({ _id: new ObjectId(String(templeId)) });
    }

    if (!suggestedTempleDoc) {
      res.status(404).json({ ok: false, error: "Suggested temple not found in DB for provided templeId" });
      return;
    }

    const templeNameRaw = suggestedTempleDoc.name || String(templeId);
    ...
    console.log(`Generating presigned URL for suggested temple ${templeId} (folder '${folderName}') blob: ${blobName}`);
    ...
    console.error("presigned_upload_suggested_temple_photo error:", err);
