...
export default async function handler(req: VercelRequest, res: VercelResponse): Promise<void> {
  console.log('Delete Temple API handler called');
  ...
  try {
    ...
    // Get temple ID from query parameters or request body
    let templeId: string;

    if (req.query.id) {
      templeId = req.query.id as string;
    } else if (req.body) {
      const bodyData = typeof req.body === 'string' ? JSON.parse(req.body) : req.body;
      templeId = bodyData.id;
    } else {
    ...
    const result = await db.collection<TempleDocument>("temples").deleteOne(
      { _id: objectId }
    );
    ...
    console.log(`Successfully deleted temple with ID: ${templeId}`);
    ...
    console.error('==================== ERROR IN DELETE TEMPLE API ====================');
