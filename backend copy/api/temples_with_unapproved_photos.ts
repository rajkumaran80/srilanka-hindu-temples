...
export default async function handler(req: VercelRequest, res: VercelResponse): Promise<void> {
  console.log('Temples with Unapproved Photos API handler called');
  ...
  try {
    ...
    console.log(`Fetching temples with unapproved photos - Page: ${page}, Limit: ${limit}`);

    // Query for temples that have unapproved_photos array with at least one item
    const query = {
      unapproved_photos: { $exists: true, $ne: [] }
    };

    // Get total count for pagination
    const totalCount = await db.collection<TempleDocument>("temples").countDocuments(query);

    // Fetch temples with pagination
    const temples = await db.collection<TempleDocument>("temples")
      .find(query)
      .skip(skip)
      .limit(limit)
      .toArray();
    ...
    console.log(`Found ${temples.length} temples with unapproved photos (total: ${totalCount})`);
    ...
    console.error('==================== ERROR IN TEMPLES WITH UNAPPROVED PHOTOS API ====================');
