...
export default async function handler(req: VercelRequest, res: VercelResponse): Promise<void> {
  ...
  try {
    ...
    const { limit, north, south, east, west, levels } = req.query;

    const query: SearchQuery & { level?: { $in: number[] } } = {};
    ...
    // If levels parameter is provided, filter by level
    if (levels) {
      const levelArray = (levels as string).split(',').map(level => parseInt(level.trim(), 10)).filter(level => !isNaN(level));
      if (levelArray.length > 0) {
        query.level = { $in: levelArray };
        console.log('Applied level filter:', levelArray);
      }
    }
