...
export default async function handler(req: VercelRequest, res: VercelResponse): Promise<void> {
  console.log('Add Unapproved Photo API handler called');
  ...
  try {
    ...
    const { templeId, photoName } = requestBody;
    ...
    if (!photoName || photoName.trim() === '') {
      console.log('Missing or empty photoName');
      return res.status(400).json({ error: 'photoName is required and cannot be empty' });
    }

    console.log(`Adding unapproved photo for temple: ${templeId}, photo: ${photoName}`);

    // Add photo name to temple's unapproved_photos array
    const updateResult = await (db.collection("suggested_temples") as any).updateOne(
    ...
    if (updateResult.modifiedCount === 0) {
      console.log(`Photo already exists in unapproved_photos for temple: ${templeId}`);
      return res.status(200).json({
        success: true,
        message: 'Photo already exists in unapproved photos',
        photoName: photoName.trim()
      });
    }

    console.log(`Unapproved photo added successfully for temple: ${templeId}`);
    console.log('Sending successful response');
    res.status(201).json({
      success: true,
      message: 'Photo added to unapproved photos successfully',
      photoName: photoName.trim()
    });

  } catch (err: unknown) {
    console.error('==================== ERROR IN ADD UNAPPROVED PHOTO API ====================');
